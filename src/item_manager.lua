...
local item_type_patterns = {
   ...
   quest = { "Global", "Glyph", "QST", "DGN", "pvp_currency", "S07_Witch_Bonus", "GamblingCurrency_Key", "S09_Arcana", "S11_MemoryFragment" },
   ...
   treasure_cache = { "Treasure_Reward_Cache", "Item_Cache" },
...
function ItemManager.check_is_boss_drop(item)
   return ItemManager.check_item_type(item, "boss_drops")
end

function ItemManager.check_is_xp_powerup(item)
   return ItemManager.check_item_type(item, "xp_powerup")
end

function ItemManager.check_is_tribute_drop(item)
...
function ItemManager.check_is_glyph_drop(item)
   return ItemManager.check_item_type(item, "glyph_drop")
end
...
function ItemManager.check_want_item(item, ignore_distance)
   ...
   if ItemManager.check_is_glyph_drop(item) then return true end
